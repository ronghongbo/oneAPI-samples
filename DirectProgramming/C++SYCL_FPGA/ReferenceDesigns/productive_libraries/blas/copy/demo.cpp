...
void test(int N, int incx, int incy) {
    ...
    auto done = t2sp::blas::row_major::copy(q_device, N, x.data(), incx, y.data(), incy);
    done.wait();
...
int main() {
#if defined(T2SP_SCOPY)
    using test_type = float;
#elif defined(T2SP_DCOPY)
    using test_type = double;
#elif defined(T2SP_CCOPY)
    using test_type = std::complex<float>;
#elif defined(T2SP_ZCOPY)
