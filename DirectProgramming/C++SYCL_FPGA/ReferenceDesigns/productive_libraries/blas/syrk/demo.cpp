...
template <typename T>
int test(device* dev, oneapi::mkl::uplo upper_lower,
...
// Create a queue bound to either the FPGA emulator or FPGA device.
    ...
    sycl::event e = t2sp::blas::row_major::syrk(fpga_queue, upper_lower, trans, n, k,
    ...
    oneapi::mkl::blas::row_major::syrk(main_queue, upper_lower, trans, n, k,
                                       alpha, A.data(), lda, beta, C.data(), ldc).wait();
