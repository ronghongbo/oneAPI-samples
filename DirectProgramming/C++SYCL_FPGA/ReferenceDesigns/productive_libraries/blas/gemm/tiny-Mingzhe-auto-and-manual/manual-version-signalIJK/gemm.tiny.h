                ...
                for (int j = 0; j < (TOTAL_J + 15) / 16; j++) {
                    for (int k = 0; k < (TOTAL_K + 15) / 16; k++) {
                        //for (int kk_ii_jj = 0; kk_ii_jj < 64; kk_ii_jj++) {
                            bool loop_i_unfinished = (i < (TOTAL_I + 15) / 16);
                            bool loop_k_start = (k == 0);
                            bool loop_k_end = (k == (TOTAL_K + -1) / 16);
                            signals_t signals = {loop_i_unfinished, loop_k_start, loop_k_end};
    ...
    /*oneapi_kernel_events.push_back(*/q.submit([&](sycl::handler &h) {
        h.single_task<class kernel_Product>([=]() {
            ...
            while (1) {
                        ...
                        bool loop_i_unfinished = signals.s[0];
                        bool loop_k_start = signals.s[1];
                        bool loop_k_end = signals.s[2];
                        for (int kk_ii_jj = 0; kk_ii_jj < 64; kk_ii_jj++) {
#pragma unroll
...
#pragma unroll
                                for (int jjj = 0; jjj < 4; jjj++) {
                                    ...
                                    Z_shreg_ = loop_k_start && kk_ii_jj / 16 == 0 ? 0.0f : sycl::ext::intel::fpga_reg(Z_shreg[0][jjj][iii]);
#pragma unroll
...
#pragma unroll
                                    for (int kkk = 0; kkk < 4; kkk++) {
                                        if (kkk == 3 && kk_ii_jj / 16 == 3 && loop_k_end/*k == (TOTAL_K + -1) / 16*/) {
                                            Z_pipe_shreg[jjj][iii * 16] = Z_shreg[0][jjj][iii];
                                        }
                                    }
                                }
                            }
                            if (kk_ii_jj % 4 == 0 && kk_ii_jj % 16 / 4 == 0 && loop_k_end/*k == (TOTAL_K + -1) / 16*/ && kk_ii_jj / 16 == 3 && loop_i_unfinished/*i < (TOTAL_I + 15) / 16*/) {
