...
typedef union {
bool __attribute__ ((aligned(4))) s[3];
struct {bool s0,  s1,  s2;};
} bool3;
using aLoader_channel = pipe_wrapper<class aLoader_channel_pipe, float, 256>;
struct aFeeder_channel_array_t { float s[3]; };
...
struct bFeeder_channel_array_t { float s[3]; };
using bFeeder_channel = pipe_wrapper<class bFeeder_channel_pipe, bFeeder_channel_array_t, 256>;
using Product_channel = pipe_wrapper<class Product_channel_pipe, float3, 256>;
using cLoader_channel = pipe_wrapper<class cLoader_channel_pipe, float3, 256>;
using Out_channel = pipe_wrapper<class Out_channel_pipe, float3, 256>;
auto sgemm(device_selector_t device_selector_v, bool p0, bool p1, float p2, float p3, struct halide_buffer_t *A_buffer, struct halide_buffer_t *B_buffer, struct halide_buffer_t *C_buffer, struct halide_buffer_t *Output_buffer) {
  ...
  if (_halide_buffer_is_bounds_query(Output_buffer)) {
    struct halide_dimension_t s3[6] = {
      ...
      {0, 1, 3, 0},
      {0, 3, 3, 0},
      {0, (B_extent_0 + 2) / 3, 9, 0},
      {0, (A_extent_1 + 2) / 3, (B_extent_0 + 2) / 3 * 9, 0},
    };
  }
  if (!(_halide_buffer_is_bounds_query(Output_buffer) || (_halide_buffer_is_bounds_query(C_buffer) || (_halide_buffer_is_bounds_query(A_buffer) || _halide_buffer_is_bounds_query(B_buffer))))) {
    ...
    struct halide_dimension_t s4[9] = {
      ...
      {0, 1, 3, 0},
      {0, 1, 3, 0},
      {0, A_serializer_k_extent_realized, 3, 0},
      {0, 1, A_serializer_k_extent_realized * 3, 0},
      {0, A_serializer_i_extent_realized, A_serializer_k_extent_realized * 3, 0},
    };
    ...
    struct s6 { void * const ucon; void * const arg; s6(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s6() { halide_device_and_host_free_as_destructor(ucon, arg); } } d0(_ucon, A_serializer_mem_channel_buffer);
    {
      ...
      }
      {
        ...
        {
          for (int i = 0; i < (A_extent_1 + 2) / 3; i++) {
            for (int k = 0; k < B_extent_1; k++) {
              for (int iii = 0; iii < 3; iii++) {
                if (i * 3 + iii < A_extent_1 && k < B_extent_1) {
                  auto _D0 = (!p0 ? k : i * 3 + iii) + (!p0 ? i * 3 + iii : k) * A_stride_1 - (A_min_1 * A_stride_1 + A_min_0);
        ...
        h.single_task<class kernel_aLoader_class>([=](){
          ...
          for (int i = 0; i < (A_extent_1 + 5) / 3; i++) {
            for (int j = 0; j < (B_extent_0 + 2) / 3; j++) {
              for (int k = 0; k < B_extent_1; k++) {
                for (int iii = 0; iii < 3; iii++) {
                  if (j == 0 && k == 0 || i < (A_extent_1 + 2) / 3) {
                    auto _D1 = addr_temp / ((B_extent_0 + 2) / 3 * B_extent_1 * 3) * B_extent_1 * 3 + addr_temp % (B_extent_1 * 3);
                    aLoader_channel::write<>(i * 3 + iii < A_extent_1 && k < B_extent_1 && i < (A_extent_1 + 2) / 3 ? A_serializer_mem_channel[_D1] : float_from_bits(0));
                  }
                  addr_temp = addr_temp + 1;
                }
              }
      ...
      oneapi_kernel_events.push_back(q_device.submit([&](sycl::handler &h){
        h.single_task<class kernel_aFeeder_class>([=](){
          ...
          float aFeeder_in_v;
          uint aFeeder_cycle;
          // OpenCL's __attribute__((memory, numbanks(4), singlepump, numwriteports(1), numreadports(1)))DB[2][4]
          [[intel::fpga_memory(), intel::numbanks(4), intel::singlepump, intel::simple_dual_port]]
          float DB[2][4];
          #pragma unroll
          for (int jjj_init = 0; jjj_init < 3; jjj_init++) {
            if (jjj_init == 0) {
          ...
          while(1) {
            ...
            for (int buf = 0; buf < 3; buf++) {
              ...
              if (buf == (int)(aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(3)) % (uint)(ADD_UINT64_T_SUFFIX(3)))) {
                DB[(bool)(aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(3)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][buf] = aFeeder_value_shreg;
              }
              if ((uint)(ADD_UINT64_T_SUFFIX(0)) < aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(3)) && aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(3)) < (uint)(ADD_UINT64_T_SUFFIX(1))) {
                aFeeder_channel_array.s[buf] = DB[!(bool)(aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(3)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][buf];
              }
            }
            if ((uint)(ADD_UINT64_T_SUFFIX(0)) < aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(3)) && aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(3)) < (uint)(ADD_UINT64_T_SUFFIX(1))) {
      ...
      struct halide_dimension_t s7[9] = {
        ...
        {0, 1, 3, 0},
        {0, 1, 3, 0},
        {0, B_serializer_k_extent_realized, 3, 0},
        {0, B_serializer_j_extent_realized, B_serializer_k_extent_realized * 3, 0},
        {0, 1, B_serializer_j_extent_realized * B_serializer_k_extent_realized * 3, 0},
      };
      ...
      struct s9 { void * const ucon; void * const arg; s9(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s9() { halide_device_and_host_free_as_destructor(ucon, arg); } } d1(_ucon, B_serializer_mem_channel_buffer);
      {
            ...
            for (int j = 0; j < (B_extent_0 + 2) / 3; j++) {
              for (int k = 0; k < B_extent_1; k++) {
                for (int jjj = 0; jjj < 3; jjj++) {
                  if (k < B_extent_1 && j * 3 + jjj < B_extent_0) {
                    auto _D2 = (!p1 ? j * 3 + jjj : k) + (!p1 ? k : j * 3 + jjj) * B_stride_1 - (B_min_1 * B_stride_1 + B_min_0);
            ...
            for (int i = 0; i < (A_extent_1 + 5) / 3; i++) {
              for (int j = 0; j < (B_extent_0 + 2) / 3; j++) {
                for (int k = 0; k < B_extent_1; k++) {
                  for (int jjj = 0; jjj < 3; jjj++) {
                    if (j == 0 && k == 0 || i < (A_extent_1 + 2) / 3) {
                      auto _D3 = addr_temp % ((B_extent_0 + 2) / 3 * B_extent_1 * 3);
                      bLoader_channel::write<>(k < B_extent_1 && j * 3 + jjj < B_extent_0 && i < (A_extent_1 + 2) / 3 ? B_serializer_mem_channel[_D3] : float_from_bits(0));
                    }
                    addr_temp = addr_temp + 1;
                  }
                }
        ...
        oneapi_kernel_events.push_back(q_device.submit([&](sycl::handler &h){
          h.single_task<class kernel_bFeeder_class>([=](){
            ...
            float DB[2][4];
            #pragma unroll
            for (int iii_init = 0; iii_init < 3; iii_init++) {
              if (iii_init == 0) {
            ...
            while(1) {
              ...
              for (int buf = 0; buf < 3; buf++) {
                ...
                if (buf == (int)(bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(3)) % (uint)(ADD_UINT64_T_SUFFIX(3)))) {
                  DB[(bool)(bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(3)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][buf] = bFeeder_value_shreg;
                }
                if ((uint)(ADD_UINT64_T_SUFFIX(0)) < bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(3)) && bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(3)) < (uint)(ADD_UINT64_T_SUFFIX(1))) {
                  bFeeder_channel_array.s[buf] = DB[!(bool)(bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(3)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][buf];
                }
              }
              if ((uint)(ADD_UINT64_T_SUFFIX(0)) < bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(3)) && bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(3)) < (uint)(ADD_UINT64_T_SUFFIX(1))) {
        ...
        oneapi_kernel_events.push_back(q_device.submit([&](sycl::handler &h){
          h.single_task<class kernel_Product_class>([=](){
            [[intel::fpga_register]] bFeeder_channel_array_t bFeeder_channel_array;
            [[intel::fpga_register]] aFeeder_channel_array_t aFeeder_channel_array;
            [[intel::fpga_register]] float Z_shreg[3][3];
            [[intel::fpga_register]] float Z_pipe_shreg[3][3];
            [[intel::fpga_register]] float Y_shreg[3];
            [[intel::fpga_register]] float X_shreg[3];
            [[intel::fpga_register]] int Z_pipe_iter;
            [[intel::fpga_register]] int Z_pipe_base;
            Z_pipe_iter = 3;
            Z_pipe_base = 0;
                ...
                for (int k = 0; k < B_extent_1; k++) {
                  ...
                  for (int iii = 0; iii < 3; iii++) {
                    ...
                    for (int jjj = 0; jjj < 3; jjj++) {
                      ...
                      Z_shreg[jjj][iii] = Z_shreg[jjj][iii] + X_shreg[iii] * Y_shreg[jjj];
                      if (k == B_extent_1 + -1) {
                        Z_pipe_shreg[jjj][iii] = Z_shreg[jjj][iii];
                      }
                    }
                  }
                  if (i < (A_extent_1 + 2) / 3 && k == B_extent_1 + -1) {
                    Z_pipe_base = Z_pipe_iter;
                  }
                  float3 Product_channel_;
                  #pragma unroll
                  for (int b_62 = 0; b_62 < 3; b_62++) {
                    Product_channel_[b_62] = Z_pipe_shreg[b_62][0];
                  }
                  if (Z_pipe_iter < Z_pipe_base + 3) {
                    Product_channel::write<>(Product_channel_);
                  }
                  #pragma unroll
                  for (int b_63 = 0; b_63 < 3; b_63++) {
                    #pragma unroll
                    for (int p_31 = 0; p_31 < 2; p_31++) {
        ...
        struct halide_dimension_t s10[6] = {
          ...
          {0, 1, 3, 0},
          {0, 3, 3, 0},
          {0, C_serializer_j_extent_realized, 9, 0},
          {0, C_serializer_i_extent_realized, C_serializer_j_extent_realized * 9, 0},
        };
        ...
        struct s12 { void * const ucon; void * const arg; s12(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s12() { halide_device_and_host_free_as_destructor(ucon, arg); } } d2(_ucon, C_serializer_mem_channel_buffer);
        {
                ...
                for (int j = 0; j < (B_extent_0 + 2) / 3; j++) {
                  for (int iii = 0; iii < 3; iii++) {
                    if (p3 != float_from_bits(0)) {
                      auto _D4 = j * 3 + (i * 3 + iii) * C_stride_1 - (C_min_1 * C_stride_1 + C_min_0);
                      auto _D5 = float3{
                        C[_D4 + 0],
                        C[_D4 + 1],
                        C[_D4 + 2]
                      };
                      C_serializer_mem_channel[addr_temp * 3 + 0] = _D5[0];
                      C_serializer_mem_channel[addr_temp * 3 + 1] = _D5[1];
                      C_serializer_mem_channel[addr_temp * 3 + 2] = _D5[2];
                    }
                    addr_temp = addr_temp + 1;
                  }
                }
            ...
            h.single_task<class kernel_cLoader_class>([=](){
              ...
              for (int i = 0; i < (A_extent_1 + 2) / 3; i++) {
                for (int j = 0; j < (B_extent_0 + 2) / 3; j++) {
                  for (int iii = 0; iii < 3; iii++) {
                    if (p3 != float_from_bits(0)) {
                      cLoader_channel::write<>(float3{
                        C_serializer_mem_channel[addr_temp * 3 + 0],
                        C_serializer_mem_channel[addr_temp * 3 + 1],
                        C_serializer_mem_channel[addr_temp * 3 + 2]
                      });
                    }
                    addr_temp = addr_temp + 1;
                  }
          ...
          oneapi_kernel_events.push_back(q_device.submit([&](sycl::handler &h){
            h.single_task<class kernel_Out_class>([=](){
              ...
              for (int i = 0; i < (A_extent_1 + 2) / 3; i++) {
                for (int j = 0; j < (B_extent_0 + 2) / 3; j++) {
                  for (int iii = 0; iii < 3; iii++) {
                    Add_shreg = (p3 == float_from_bits(0) ? float3{float_from_bits(0)} : cLoader_channel::read<>() * float3{p3}) + Product_channel::read<>() * float3{p2};
          ...
          struct halide_dimension_t s13[6] = {
            ...
            {0, 1, 3, 0},
            {0, 3, 3, 0},
            {0, unloader_j_extent_realized, 9, 0},
            {0, unloader_i_extent_realized, unloader_j_extent_realized * 9, 0},
          };
                ...
                for (int i = 0; i < (A_extent_1 + 2) / 3; i++) {
                  for (int j = 0; j < (B_extent_0 + 2) / 3; j++) {
                    for (int iii = 0; iii < 3; iii++) {
                      ...
                      unloader_mem_channel[addr_temp * 3 + 0] = _D6[0];
                      unloader_mem_channel[addr_temp * 3 + 1] = _D6[1];
                      unloader_mem_channel[addr_temp * 3 + 2] = _D6[2];
                      addr_temp = addr_temp + 1;
                    }
                  }
                }
              ...
              float *Output = (float*)(Output_buffer->host);
              {
                for (int i = 0; i < (A_extent_1 + 2) / 3; i++) {
                  for (int j = 0; j < (B_extent_0 + 2) / 3; j++) {
                    for (int iii = 0; iii < 3; iii++) {
                      auto _D7 = float3{
                        unloader_mem_channel[addr_temp * 3 + 0],
                        unloader_mem_channel[addr_temp * 3 + 1],
                        unloader_mem_channel[addr_temp * 3 + 2]
                      };
