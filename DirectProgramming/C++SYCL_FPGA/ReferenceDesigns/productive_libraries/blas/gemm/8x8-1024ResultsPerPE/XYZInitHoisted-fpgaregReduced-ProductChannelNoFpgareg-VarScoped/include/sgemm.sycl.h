...
struct aFeeder_channel_array_t { float16 s[8]; };
...
auto sgemm(device_selector_t device_selector_v, bool p0, bool p1, float p2, float p3, struct halide_buffer_t *A_buffer, struct halide_buffer_t *B_buffer, struct halide_buffer_t *C_buffer, struct halide_buffer_t *Output_buffer) {
  ...
  if (_halide_buffer_is_bounds_query(Output_buffer)) {
    struct halide_dimension_t s3[6] = {
      ...
      {0, 32, 256, 0},
      {0, 8, 8192, 0},
      {0, (B_extent_0 + 255) / 256, 65536, 0},
      {0, (A_extent_1 + 255) / 256, (B_extent_0 + 255) / 256 * 65536, 0},
    };
  }
  if (!(_halide_buffer_is_bounds_query(Output_buffer) || (_halide_buffer_is_bounds_query(C_buffer) || (_halide_buffer_is_bounds_query(A_buffer) || _halide_buffer_is_bounds_query(B_buffer))))) {
    ...
    struct halide_dimension_t s4[9] = {
      ...
      {0, 32, 128, 0},
      {0, 32, 4096, 0},
      {0, A_serializer_k_extent_realized, 131072, 0},
      {0, 1, A_serializer_k_extent_realized * 131072, 0},
      {0, A_serializer_i_extent_realized, A_serializer_k_extent_realized * 131072, 0},
    };
    ...
    struct s6 { void * const ucon; void * const arg; s6(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s6() { halide_device_and_host_free_as_destructor(ucon, arg); } } d0(_ucon, A_serializer_mem_channel_buffer);
    {
      ...
      }
      {
        ...
        {
          for (int i = 0; i < (A_extent_1 + 255) / 256; i++) {
            for (int k = 0; k < (B_extent_1 + 511) / 512; k++) {
              for (int kk_ii_iii_kkk = 0; kk_ii_iii_kkk < 131072; kk_ii_iii_kkk++) {
                if (i * 256 + (kk_ii_iii_kkk % 128 / 16 + kk_ii_iii_kkk % 4096 / 128 * 8) < A_extent_1 && (kk_ii_iii_kkk / 4096 + k * 32) * 16 < B_extent_1) {
                  auto _D0 = (!p0 ? k * 512 + (kk_ii_iii_kkk / 4096 * 16 + kk_ii_iii_kkk % 16) : i * 256 + (kk_ii_iii_kkk % 128 / 16 + kk_ii_iii_kkk % 4096 / 128 * 8)) + (!p0 ? i * 256 + (kk_ii_iii_kkk % 128 / 16 + kk_ii_iii_kkk % 4096 / 128 * 8) : k * 512 + (kk_ii_iii_kkk / 4096 * 16 + kk_ii_iii_kkk % 16)) * A_stride_1 - (A_min_1 * A_stride_1 + A_min_0);
        ...
        h.single_task<class kernel_aLoader_class>([=](){
          ...
          for (int i = 0; i < (A_extent_1 + 511) / 256; i++) {
            for (int j = 0; j < (B_extent_0 + 255) / 256; j++) {
              for (int k = 0; k < (B_extent_1 + 511) / 512; k++) {
                for (int kk_ii_iii = 0; kk_ii_iii < 8192; kk_ii_iii++) {
                  if (j == 0 && k == 0 || i < (A_extent_1 + 255) / 256) {
                    auto _D1 = (addr_temp / ((B_extent_0 + 255) / 256 * ((B_extent_1 + 511) / 512) * 8192) * ((B_extent_1 + 511) / 512) * 8192 + addr_temp % ((B_extent_1 + 511) / 512 * 8192)) * 16;
                    aLoader_channel::write<>(i * 256 + (kk_ii_iii % 256 / 8 * 8 + kk_ii_iii % 8) < A_extent_1 && (kk_ii_iii / 256 + k * 32) * 16 < B_extent_1 && i < (A_extent_1 + 255) / 256 ? float16{
      ...
      oneapi_kernel_events.push_back(q_device.submit([&](sycl::handler &h){
        h.single_task<class kernel_aFeeder_class>([=](){
          ...
          for (int jjj_init = 0; jjj_init < 8; jjj_init++) {
            if (jjj_init == 0) {
              aFeeder_cycle = (uint)(ADD_UINT64_T_SUFFIX(24576));
            }
          }
          while(1) {
            if ((uint)(ADD_UINT64_T_SUFFIX(24576)) <= aFeeder_cycle % (uint)(ADD_UINT64_T_SUFFIX(32768))) {
            ...
            for (int buf = 0; buf < 8; buf++) {
              ...
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[0]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[1]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[2]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[3]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[4]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[5]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[6]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[7]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[8]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[9]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[10]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[11]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[12]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[13]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[14]),
              sycl::ext::intel::fpga_reg(aFeeder_value_shreg[15])
              };
              aFeeder_time_stamp_shreg = sycl::ext::intel::fpga_reg(aFeeder_time_stamp_shreg);
              if ((uint)(ADD_UINT64_T_SUFFIX(24576)) <= aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(32768))) {
                if (buf == (int)((aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(32768)) - (uint)(ADD_UINT64_T_SUFFIX(24576))) % (uint)(ADD_UINT64_T_SUFFIX(8)))) {
                  DB[(bool)(aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(32768)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][(int)(aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(32768)) - (uint)(ADD_UINT64_T_SUFFIX(24576))) / 256][(int)(aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(32768)) - (uint)(ADD_UINT64_T_SUFFIX(24576))) / 8 % 32][buf] = aFeeder_value_shreg;
                }
              }
      ...
      struct halide_dimension_t s7[9] = {
        ...
        {0, 1, 4096, 0},
        {0, 32, 4096, 0},
        {0, B_serializer_k_extent_realized, 131072, 0},
        {0, B_serializer_j_extent_realized, B_serializer_k_extent_realized * 131072, 0},
        {0, 1, B_serializer_j_extent_realized * B_serializer_k_extent_realized * 131072, 0},
      };
      ...
      struct s9 { void * const ucon; void * const arg; s9(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s9() { halide_device_and_host_free_as_destructor(ucon, arg); } } d1(_ucon, B_serializer_mem_channel_buffer);
      {
                ...
                for (int k = 0; k < (B_extent_1 + 511) / 512; k++) {
                  for (int kk_jj_jjj = 0; kk_jj_jjj < 8192; kk_jj_jjj++) {
                    if (j == 0 && k == 0 || i < (A_extent_1 + 255) / 256) {
                      ...
                      bLoader_channel::write<>((kk_jj_jjj / 256 + k * 32) * 16 < B_extent_1 && j * 256 + (kk_jj_jjj % 256 / 8 * 8 + kk_jj_jjj % 8) < B_extent_0 && i < (A_extent_1 + 255) / 256 ? float16{
          ...
          h.single_task<class kernel_bFeeder_class>([=](){
            ...
            while(1) {
              ...
              for (int buf = 0; buf < 8; buf++) {
                ...
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[0]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[1]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[2]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[3]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[4]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[5]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[6]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[7]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[8]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[9]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[10]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[11]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[12]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[13]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[14]),
                sycl::ext::intel::fpga_reg(bFeeder_value_shreg[15])
                };
                bFeeder_time_stamp_shreg = sycl::ext::intel::fpga_reg(bFeeder_time_stamp_shreg);
                  ...
                  for (int kk_ii_jj = 0; kk_ii_jj < 32768; kk_ii_jj++) {
                    ...
                    for (int iii = 0; iii < 8; iii++) {
                      ...
                      for (int jjj = 0; jjj < 8; jjj++) {
                        ...
                        [[intel::fpga_register]] float Z_shreg_;
                        Z_shreg_ = Z_shreg[0][jjj][iii];
                        #pragma unroll
                    ...
                    if (kk_ii_jj % 32 == 0 && kk_ii_jj % 1024 / 32 == 0 && k == (B_extent_1 + -1) / 512 && kk_ii_jj / 1024 == 31 && i < (A_extent_1 + 255) / 256) {
                    ...
                    for (int b_62 = 0; b_62 < 8; b_62++) {
                      ...
                      for (int b_62_dummy = 0; b_62_dummy < 8; b_62_dummy++) {
                        Product_channel_[b_62_dummy] = sycl::ext::intel::fpga_reg(Product_channel_[b_62_dummy]);
                      }*/
                    }
                    if (Z_pipe_iter < Z_pipe_base + 8192) {
                    ...
                    for (int b_63 = 0; b_63 < 8; b_63++) {
                      ...
                      for (int p_31 = 0; p_31 < 7; p_31++) {
                        ...
                        Z_pipe_shreg[b_63][p_31 * 1024 + 1023] = sycl::ext::intel::fpga_reg(Z_pipe_shreg[b_63][p_31 * 1024 + 1024]);
                      }
                    }
        ...
        struct halide_dimension_t s10[6] = {
          ...
          {0, 32, 256, 0},
          {0, 8, 8192, 0},
          {0, C_serializer_j_extent_realized, 65536, 0},
          {0, C_serializer_i_extent_realized, C_serializer_j_extent_realized * 65536, 0},
        };
        ...
        struct s12 { void * const ucon; void * const arg; s12(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s12() { halide_device_and_host_free_as_destructor(ucon, arg); } } d2(_ucon, C_serializer_mem_channel_buffer);
        {
          ...
          }
          {
            ...
            C_serializer_mem_channel = (float*)(C_serializer_mem_channel_buffer->host);
            {
              for (int i = 0; i < (A_extent_1 + 255) / 256; i++) {
                for (int j = 0; j < (B_extent_0 + 255) / 256; j++) {
                  for (int iii_ii_jj = 0; iii_ii_jj < 8192; iii_ii_jj++) {
                    if (p3 != float_from_bits(0)) {
                      auto _D4 = (j * 32 + iii_ii_jj % 32) * 8 + (i * 256 + (iii_ii_jj / 1024 + iii_ii_jj % 1024 / 32 * 8)) * C_stride_1 - (C_min_1 * C_stride_1 + C_min_0);
                      auto _D5 = float8{
                        C[_D4 + 0],
                        C[_D4 + 1],
                        C[_D4 + 2],
          ...
          struct halide_dimension_t s13[6] = {
            ...
            {0, 32, 256, 0},
            {0, 8, 8192, 0},
            {0, unloader_j_extent_realized, 65536, 0},
            {0, unloader_i_extent_realized, unloader_j_extent_realized * 65536, 0},
          };
