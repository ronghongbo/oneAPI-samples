...
using FLOAT8 = fpga_tools::NTuple<float, 8>;
using aLoader_channel = pipe_wrapper<class aLoader_channel_pipe, float, 256>;
using aFeeder_channel = pipe_wrapper<class aFeeder_channel_pipe, FLOAT8, 256>;
using bLoader_channel = pipe_wrapper<class bLoader_channel_pipe, float, 256>;
using bFeeder_channel = pipe_wrapper<class bFeeder_channel_pipe, FLOAT8, 256>;
            ...
            while(1) {
              ...
              fpga_tools::UnrolledLoop<8>([&](auto buf) {
                ...
                if ((uint)(ADD_UINT64_T_SUFFIX(0)) < bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(8)) && bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(8)) < (uint)(ADD_UINT64_T_SUFFIX(1))) {
                  bFeeder_channel_array.template get<buf>() = DB[!(bool)(bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(8)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][buf];
                }
              });
              if ((uint)(ADD_UINT64_T_SUFFIX(0)) < bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(8)) && bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(8)) < (uint)(ADD_UINT64_T_SUFFIX(1))) {
                ...
                for (int k = 0; k < B_extent_1; k++) {
                  ...
                  fpga_tools::UnrolledLoop<8>([&](auto iii) {
                    fpga_tools::UnrolledLoop<8>([&](auto jjj) {
                      ...
                      X_shreg.template get<iii>() = sycl::ext::intel::fpga_reg(X_shreg.template get<iii>());
                      ...
                      Y_shreg.template get<jjj>() = sycl::ext::intel::fpga_reg(X_shreg.template get<jjj>());

                      
                      // Hoist out initialization
                      //  Z_shreg[jjj][iii] = (k == 0 ? float_from_bits(0) : Z_shreg[jjj][iii]) + X_shreg[iii] * Y_shreg[jjj];
                      Z_shreg[jjj][iii] = X_shreg.template get<iii>() * Y_shreg.template get<jjj>() + Z_shreg[jjj][iii];
                      if (k == B_extent_1 + -1) {
