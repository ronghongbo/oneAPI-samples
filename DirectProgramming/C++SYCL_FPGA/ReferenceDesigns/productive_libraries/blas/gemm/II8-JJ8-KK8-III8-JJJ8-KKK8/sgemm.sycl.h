...
using aLoader_channel = pipe_wrapper<class aLoader_channel_pipe, float8, 256>;
struct aFeeder_channel_array_t { float8 s[8]; };
using aFeeder_channel = pipe_wrapper<class aFeeder_channel_pipe, aFeeder_channel_array_t, 256>;
using bLoader_channel = pipe_wrapper<class bLoader_channel_pipe, float8, 256>;
struct bFeeder_channel_array_t { float8 s[8]; };
...
auto sgemm(device_selector_t device_selector_v, bool p0, bool p1, float p2, float p3, struct halide_buffer_t *A_buffer, struct halide_buffer_t *B_buffer, struct halide_buffer_t *C_buffer, struct halide_buffer_t *Output_buffer) {
  ...
  if (_halide_buffer_is_bounds_query(Output_buffer)) {
    struct halide_dimension_t s3[6] = {
      ...
      {0, 8, 64, 0},
      {0, 8, 512, 0},
      {0, (B_extent_0 + 63) / 64, 4096, 0},
      {0, (A_extent_1 + 63) / 64, (B_extent_0 + 63) / 64 * 4096, 0},
    };
  }
  if (!(_halide_buffer_is_bounds_query(Output_buffer) || (_halide_buffer_is_bounds_query(C_buffer) || (_halide_buffer_is_bounds_query(A_buffer) || _halide_buffer_is_bounds_query(B_buffer))))) {
    ...
    struct halide_dimension_t s4[9] = {
      ...
      {0, 8, 64, 0},
      {0, 8, 512, 0},
      {0, (B_extent_1 + 63) / 64, 4096, 0},
      {0, 1, (B_extent_1 + 63) / 64 * 4096, 0},
      {0, (A_extent_1 + 63) / 64, (B_extent_1 + 63) / 64 * 4096, 0},
    };
    ...
    struct s6 { void * const ucon; void * const arg; s6(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s6() { halide_device_and_host_free_as_destructor(ucon, arg); } } d0(_ucon, A_serializer_mem_channel_buffer);
    {
      ...
      }
      {
        ...
        {
          for (int i = 0; i < (A_extent_1 + 63) / 64; i++) {
            for (int k = 0; k < (B_extent_1 + 63) / 64; k++) {
              for (int kk_ii_iii_kkk = 0; kk_ii_iii_kkk < 4096; kk_ii_iii_kkk++) {
                if (i * 64 + (kk_ii_iii_kkk % 64 / 8 + kk_ii_iii_kkk % 512 / 64 * 8) < A_extent_1 && (kk_ii_iii_kkk / 512 + k * 8) * 8 < B_extent_1) {
                  auto _D0 = (!p0 ? k * 64 + (kk_ii_iii_kkk / 512 * 8 + kk_ii_iii_kkk % 8) : i * 64 + (kk_ii_iii_kkk % 64 / 8 + kk_ii_iii_kkk % 512 / 64 * 8)) + (!p0 ? i * 64 + (kk_ii_iii_kkk % 64 / 8 + kk_ii_iii_kkk % 512 / 64 * 8) : k * 64 + (kk_ii_iii_kkk / 512 * 8 + kk_ii_iii_kkk % 8)) * A_stride_1 - (A_min_1 * A_stride_1 + A_min_0);
        ...
        h.single_task<class kernel_aLoader_class>([=](){
          ...
          for (int i = 0; i < (A_extent_1 + 127) / 64; i++) {
            for (int j = 0; j < (B_extent_0 + 63) / 64; j++) {
              for (int k = 0; k < (B_extent_1 + 63) / 64; k++) {
                for (int kk_ii_iii = 0; kk_ii_iii < 512; kk_ii_iii++) {
                  if (j == 0 && k == 0 || i < (A_extent_1 + 63) / 64) {
                    auto _D1 = (addr_temp / ((B_extent_0 + 63) / 64 * ((B_extent_1 + 63) / 64) * 512) * ((B_extent_1 + 63) / 64) * 512 + addr_temp % ((B_extent_1 + 63) / 64 * 512)) * 8;
                    aLoader_channel::write<>(i * 64 + (kk_ii_iii % 64 / 8 * 8 + kk_ii_iii % 8) < A_extent_1 && (kk_ii_iii / 64 + k * 8) * 8 < B_extent_1 && i < (A_extent_1 + 63) / 64 ? float8{
      ...
      oneapi_kernel_events.push_back(q_device.submit([&](sycl::handler &h){
        h.single_task<class kernel_aFeeder_class>([=](){
          ...
          float8 aFeeder_value_shreg;
          uint32_t aFeeder_time_stamp_shreg;
          float8 aFeeder_in_v;
          uint aFeeder_cycle;
          // OpenCL's __attribute__((memory, numbanks(8), singlepump, numwriteports(1), numreadports(1)))DB[2][8][8][8]
          ...
          while(1) {
            ...
            for (int buf = 0; buf < 8; buf++) {
              ...
              if (buf == (int)(aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512)) % (uint)(ADD_UINT64_T_SUFFIX(8)))) {
                DB[(bool)(aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(512)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][(int)(aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512))) / 64][(int)(aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512))) / 8 % 8][buf] = aFeeder_value_shreg;
              }
              if ((uint)(ADD_UINT64_T_SUFFIX(0)) < aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(512))) {
                aFeeder_channel_array.s[buf] = DB[!(bool)(aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(512)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][(int)(aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512))) / 64][(int)(aFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512))) / 8 % 8][buf];
              }
            }
            if ((uint)(ADD_UINT64_T_SUFFIX(0)) < aFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(512))) {
      ...
      int32_t B_serializer_mem_channel_stride_8_s = (B_extent_0 + 63) / 64 * ((B_extent_1 + 63) / 64);
      halide_buffer_t b1;
      struct halide_dimension_t s7[9] = {
        ...
        {0, 8, 512, 0},
        {0, (B_extent_1 + 63) / 64, 4096, 0},
        {0, (B_extent_0 + 63) / 64, (B_extent_1 + 63) / 64 * 4096, 0},
        {0, 1, B_serializer_mem_channel_stride_8_s * 4096, 0},
      };
      ...
      struct s9 { void * const ucon; void * const arg; s9(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s9() { halide_device_and_host_free_as_destructor(ucon, arg); } } d1(_ucon, B_serializer_mem_channel_buffer);
      {
        ...
        }
        {
          ...
          B_serializer_mem_channel = (float*)(B_serializer_mem_channel_buffer->host);
          {
            for (int j = 0; j < (B_extent_0 + 63) / 64; j++) {
              for (int k = 0; k < (B_extent_1 + 63) / 64; k++) {
                for (int kk_jj_jjj = 0; kk_jj_jjj < 512; kk_jj_jjj++) {
                  if ((kk_jj_jjj / 64 + k * 8) * 8 < B_extent_1 && j * 64 + (kk_jj_jjj % 64 / 8 * 8 + kk_jj_jjj % 8) < B_extent_0) {
                    auto _D2 = (!p1 ? int8{j * 64 + (kk_jj_jjj % 64 / 8 * 8 + kk_jj_jjj % 8)} : (kk_jj_jjj / 64 + k * 8) * 8 + 1 * int8{0, 1, 2, 3, 4, 5, 6, 7}) + (!p1 ? (kk_jj_jjj / 64 + k * 8) * 8 + 1 * int8{0, 1, 2, 3, 4, 5, 6, 7} : int8{j * 64 + (kk_jj_jjj % 64 / 8 * 8 + kk_jj_jjj % 8)}) * int8{B_stride_1} - (int8{B_min_1 * B_stride_1 + B_min_0});
                    float8 _V0;
                    _V0[0] = ((float*)B)[_D2[0]];
                    _V0[1] = ((float*)B)[_D2[1]];
                    _V0[2] = ((float*)B)[_D2[2]];
                    ...
                    _V0[6] = ((float*)B)[_D2[6]];
                    _V0[7] = ((float*)B)[_D2[7]];
                    B_serializer_mem_channel[addr_temp * 8 + 0] = _V0[0];
                    B_serializer_mem_channel[addr_temp * 8 + 1] = _V0[1];
                    B_serializer_mem_channel[addr_temp * 8 + 2] = _V0[2];
                    B_serializer_mem_channel[addr_temp * 8 + 3] = _V0[3];
                    B_serializer_mem_channel[addr_temp * 8 + 4] = _V0[4];
                    B_serializer_mem_channel[addr_temp * 8 + 5] = _V0[5];
                    B_serializer_mem_channel[addr_temp * 8 + 6] = _V0[6];
                    B_serializer_mem_channel[addr_temp * 8 + 7] = _V0[7];
                  }
                  addr_temp = addr_temp + 1;
                }
              }
            ...
            for (int i = 0; i < (A_extent_1 + 127) / 64; i++) {
              for (int j = 0; j < (B_extent_0 + 63) / 64; j++) {
                for (int k = 0; k < (B_extent_1 + 63) / 64; k++) {
                  for (int kk_jj_jjj = 0; kk_jj_jjj < 512; kk_jj_jjj++) {
                    if (j == 0 && k == 0 || i < (A_extent_1 + 63) / 64) {
                      auto _D3 = addr_temp % ((B_extent_0 + 63) / 64 * ((B_extent_1 + 63) / 64) * 512) * 8;
                      bLoader_channel::write<>((kk_jj_jjj / 64 + k * 8) * 8 < B_extent_1 && j * 64 + (kk_jj_jjj % 64 / 8 * 8 + kk_jj_jjj % 8) < B_extent_0 && i < (A_extent_1 + 63) / 64 ? float8{
          ...
          h.single_task<class kernel_bFeeder_class>([=](){
            ...
            while(1) {
              ...
              for (int buf = 0; buf < 8; buf++) {
                ...
                if (buf == (int)(bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512)) % (uint)(ADD_UINT64_T_SUFFIX(8)))) {
                  DB[(bool)(bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(512)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][(int)(bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512))) / 64][(int)(bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512))) / 8 % 8][buf] = bFeeder_value_shreg;
                }
                if ((uint)(ADD_UINT64_T_SUFFIX(0)) < bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(512))) {
                  bFeeder_channel_array.s[buf] = DB[!(bool)(bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(512)) % (uint)(ADD_UINT64_T_SUFFIX(2)))][(int)(bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512))) / 64][(int)(bFeeder_time_stamp_shreg % (uint)(ADD_UINT64_T_SUFFIX(512))) % 8][buf];
                }
              }
              if ((uint)(ADD_UINT64_T_SUFFIX(0)) < bFeeder_time_stamp_shreg / (uint)(ADD_UINT64_T_SUFFIX(512))) {
        ...
        struct s12 { void * const ucon; void * const arg; s12(void *ucon, void *a) : ucon(ucon), arg((void *)a) {} ~s12() { halide_device_and_host_free_as_destructor(ucon, arg); } } d2(_ucon, C_serializer_mem_channel_buffer);
        {
          ...
          }
          {
            ...
            C_serializer_mem_channel = (float*)(C_serializer_mem_channel_buffer->host);
            {
              for (int i = 0; i < (A_extent_1 + 63) / 64; i++) {
                for (int j = 0; j < (B_extent_0 + 63) / 64; j++) {
                  for (int iii_ii_jj = 0; iii_ii_jj < 512; iii_ii_jj++) {
                    if (p3 != float_from_bits(0)) {
                      auto _D4 = (j * 8 + iii_ii_jj % 8) * 8 + (i * 64 + (iii_ii_jj / 64 + iii_ii_jj % 64 / 8 * 8)) * C_stride_1 - (C_min_1 * C_stride_1 + C_min_0);
                      auto _D5 = float8{
                        C[_D4 + 0],
                        C[_D4 + 1],
                        C[_D4 + 2],
                ...
                for (int i = 0; i < (A_extent_1 + 63) / 64; i++) {
                  for (int j = 0; j < (B_extent_0 + 63) / 64; j++) {
                    for (int iii_ii_jj = 0; iii_ii_jj < 512; iii_ii_jj++) {
                      ...
                      auto _D8 = i * Output_stride_5 + (j * Output_stride_4 + (iii_ii_jj / 64 * Output_stride_3 + (iii_ii_jj % 8 * Output_stride_1 + iii_ii_jj % 64 / 8 * Output_stride_2))) - (Output_min_5 * Output_stride_5 + (Output_min_4 * Output_stride_4 + (Output_min_3 * Output_stride_3 + (Output_min_2 * Output_stride_2 + (Output_min_1 * Output_stride_1 + Output_min_0)))));
                      Output[_D8 + 0] = _D7[0];
                      Output[_D8 + 1] = _D7[1];
                      Output[_D8 + 2] = _D7[2];
                      Output[_D8 + 3] = _D7[3];
