              ...
              for (int j = (HalfSpaceOut ? i : 0); j < (((C_extent_0 + 3)) / 4); j++) {
                ...
                for (int k = 0; k < ((serializer_k_extent_realized_s + 3)) >> 2; k++) {
                  for (int kk = 0; kk < 2; kk++) {
                    for (int ii = 0; ii < 2; ii++) {
                      for (int iii = 0; iii < 2; iii++) {
                        auto _D8 = addr_temp*2;//((((((addr_temp / ((((((serializer_k_extent_realized_s + 3)) >> 2) * (((((C_extent_0 + 3)) >> 2) - ((HalfSpaceOut ? i : 0))))) * 8))) * (((serializer_k_extent_realized_s + 3)) >> 2)) * 8) + (addr_temp % (((((serializer_k_extent_realized_s + 3)) >> 2) * 8))))) * 2);
...
;
          ...
          oneapi_kernel_events.push_back(q_device.submit([&](sycl::handler &h){
            h.single_task<class kernel_DB_class>([=](){
              int addr_temp;
              addr_temp = 0;
              int K = ((serializer_k_extent_realized_s + 3)) >> 2;
              int KK=2;
              int JJ=2;
              int JJJ=2;
              int KKK=2;
              for (int i = 0; i < ((C_extent_1 + 3)) >> 2; i++) {
                addr_temp = (HalfSpaceOut ? i : 0) * K * KK * JJ * JJJ;
                for (int j = (HalfSpaceOut ? i : 0); j < (((C_extent_0 + 3)) / 4); j++) {
                  for (int k = 0; k < ((serializer_k_extent_realized_s + 3)) >> 2; k++) {
                    for (int kk = 0; kk < 2; kk++) {
                      for (int jj = 0; jj < 2; jj++) {
                        for (int jjj = 0; jjj < 2; jjj++) {
                          auto _D13 = addr_temp * 2;//((addr_temp % ((((((serializer_k_extent_realized_s + 3)) >> 2) * (((((C_extent_0 + 3)) >> 2) - ((HalfSpaceOut ? i : 0))))) * 8))) * 2);
          ...
          oneapi_kernel_events.push_back(q_device.submit([&](sycl::handler &h){
            h.single_task<class kernel_SignalGenerator_class>([=](){
              for (int i = 0; i < ((C_extent_1 + 7)) >> 2; i++) {
                for (int j = (HalfSpaceOut ? ((i < ((C_extent_1 + 3)) >> 2) ? i : i - 1) : 0); j < (((C_extent_0 + 3)) / 4); j++) {
                  for (int k = 0; k < ((serializer_k_extent_realized_s + 3)) >> 2; k++) {
                    for (int kk = 0; kk < 2; kk++) {
                      for (int ii = 0; ii < 2; ii++) {
                        for (int jj = 0; jj < 2; jj++) {
              ...
              for (int i = 0; i < ((C_extent_1 + 3)) >> 2; i++) {
                for (int j = (HalfSpaceOut ? i : 0); j < (((C_extent_0 + 3)) / 4); j++) {
                  for (int iii = 0; iii < 2; iii++) {
                    for (int ii = 0; ii < 2; ii++) {
                      for (int jj = 0; jj < 2; jj++) {
                          complexf2 product = Product_channel::read<>();

                          #define C_row_idx          (iii + 2 * ii + 2 * 2 * i)
                          #define C_col_idx          (jjj + 2 * jj + 2 * 2 * j)
                          #define reduction_idx      (kkk + 2 * kk + 2 * 2 * k)
                          int jjj = 0;
                          PRINTF("NewProduct(%d, %d) Product=(%f, %f)\n",
                                  C_row_idx, C_col_idx, product[0].real(), product[0].imag());
                          jjj = 1;
                          PRINTF("NewProduct(%d, %d) Product=(%f, %f)\n",
                                  C_row_idx, C_col_idx, product[1].real(), product[1].imag());

                        complexf2 Add_shreg;
                        Add_shreg = (DC_channel::read<>() + (product * alpha));
                      ...
                      for (int ii = 0; ii < 2; ii++) {
                        for (int jj = 0; jj < 2; jj++) {
                          ...
                          if (C_row_idx <= C_col_idx) {
                              PRINTF("NewResult[%d, %d](%f, %f)\n",
                                  C_row_idx, C_col_idx,
                                  _D14[0].real(), _D14[0].imag());
                          }
                          jjj=1;
                          if (C_row_idx <= C_col_idx) {
                              PRINTF("NewResult[%d, %d](%f, %f)\n",
                                  C_row_idx, C_col_idx,
                                  _D14[1].real(), _D14[1].imag());
                          }
