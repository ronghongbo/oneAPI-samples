    ...
    try {
        switch (layout) {
            ...
            case oneapi::mkl::layout::row_major:
                done = t2sp::blas::row_major::symm(fpga_queue, left_right, upper_lower, m, n,
                                                   alpha, A.data(), lda, B.data(), ldb, beta,
...
TEST_P(SymmUsmTests, RealSinglePrecision) {
    ...
    EXPECT_TRUEORSKIP(test<float>(std::get<0>(GetParam()), std::get<1>(GetParam()),
                                  oneapi::mkl::side::left, oneapi::mkl::uplo::lower, 72, 28, 101,
                                  102, 103, alpha, beta));
#elif defined(T2SP_TEST_1)
    EXPECT_TRUEORSKIP(test<float>(std::get<0>(GetParam()), std::get<1>(GetParam()),
                                  oneapi::mkl::side::right, oneapi::mkl::uplo::lower, 72, 28, 101,
                                  102, 103, alpha, beta));
#elif defined(T2SP_TEST_2)
    EXPECT_TRUEORSKIP(test<float>(std::get<0>(GetParam()), std::get<1>(GetParam()),
                                  oneapi::mkl::side::left, oneapi::mkl::uplo::upper, 72, 28, 101,
                                  102, 103, alpha, beta));
#elif defined(T2SP_TEST_3)
    EXPECT_TRUEORSKIP(test<float>(std::get<0>(GetParam()), std::get<1>(GetParam()),
                                  oneapi::mkl::side::right, oneapi::mkl::uplo::upper, 72, 28, 101,
                                  102, 103, alpha, beta));
#endif
}
